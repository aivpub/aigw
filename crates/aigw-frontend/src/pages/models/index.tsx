...
function extractCost(info: Record<string, unknown>): {
  input: number | null;
  output: number | null;
  ...
  const output =
    typeof info.output_cost_per_token === "number"
      ...
      : null;
  const cacheRead =
    typeof info.cache_read_input_token_cost === "number"
      ? info.cache_read_input_token_cost * 1_000_000
      : null;
  const cacheCreate =
    typeof info.cache_creation_input_token_cost === "number"
...
export function ModelsPage() {
  ...
  const totalPages =
    data?.total_pages ??
    (models.length === 0 ? 1 : Math.ceil(totalCount / pageSize));
  ...
  const { data: deletedData, isLoading: deletedLoading } =
    useQuery<DeletedModelListResponse>({
      queryKey: ["deleted-models", deletedPage, deletedPageSize],
      queryFn: () =>
        apiGet(
          `/model/deleted?page=${deletedPage}&page_size=${deletedPageSize}`,
        ),
  ...
  const handleDeleteClick = useCallback(
    (model: ModelItem, e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await apiDelete(
        `/model/delete?model_id=${encodeURIComponent(deletingModel.model_id)}`,
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          <TabsList>
            <TabsTrigger value="model-groups">
              {t("models.tabModelGroups")}
            </TabsTrigger>
            <TabsTrigger value="credentials">
              {t("models.tabCredentials")}
            </TabsTrigger>
          ...
          {viewMode === "deleted" && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle>
                  {t("models.deletedModels")} ({deletedTotalCount})
                </CardTitle>
              </CardHeader>
              <CardContent>
                {deletedLoading ? (
                    ...
                    <div className="hidden md:block">
                      <Table>
                        ...
                        <TableBody>
                          {deletedModels.map((m) => {
                            const provider =
                              typeof m.litellm_params?.custom_llm_provider ===
                              "string"
                                ? m.litellm_params.custom_llm_provider
                          ...
                          <Card key={m.id}>
                            <CardContent className="p-4 space-y-2">
                              ...
                              <div className="text-xs text-muted-foreground">
                                {t("models.detailId")} {m.model_id} |{" "}
                                {t("models.mobileProvider")} {provider}
                              </div>
                              <div className="text-xs text-muted-foreground">
                                {t("models.table.deletedAt")}:{" "}
                                {formatDate(m.deleted_at)}
                    ...
                    <TableBody>
                      {isLoading
                        ...
                        : filteredModels.map((model) => {
                            ...
                            const upstream = extractModelType(
                              model.litellm_params,
                            );
                                    ...
                                    >
                                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                          ...
                                          <div className="rounded-md border bg-card p-3 font-mono text-xs leading-relaxed overflow-auto max-h-64">
                                            {Object.entries(model.model_info)
                                              .length === 0
                                              ? t("models.empty")
                                              : Object.entries(
                                                  model.model_info,
                          ...
                          <Card key={model.model_id}>
                            <CardContent className="p-4 space-y-2">
                              ...
                              <div className="flex items-center justify-between text-xs text-muted-foreground">
                                ...
                                <span>
                                  {t("models.mobileUpstream")} {upstream}
                                </span>
                              </div>
                              {cost.input !== null && (
                                <div className="text-xs text-muted-foreground">
                                  ...
                                  {t("models.costInput")}
                                  {cost.output !== null &&
                                    ` / $${cost.output.toFixed(4)} ${t("models.costOutput")}`}
                                </div>
                              )}
                                ...
                                <div className="space-y-3 pt-2 border-t">
                                  <div>
                                    ...
                                    <div className="rounded-md border bg-muted/30 p-2 font-mono text-xs leading-relaxed max-h-40 overflow-auto">
                                      {Object.entries(model.litellm_params)
                                        .length === 0
                                        ? t("models.empty")
                                        : Object.entries(
                                            model.litellm_params,
