...
async fn write_pg_row(pool: &PgPool, table: &str, rows: &[UnifiedRow]) -> anyhow::Result<usize> {
    ...
    let type_map: std::collections::HashMap<&str, &str> =
        col_types.iter().map(|(n, t)| (n.as_str(), t.as_str())).collect();
    ...
    match v {
        ...
        Value::Number(n) => {
            ...
            if ty == "boolean" || ty == "bool" {
                ...
                if n.as_i64().map(|i| i != 0).unwrap_or(false) { "true".to_string() } else { "false".to_string() }
            } else if ty == "bigint" || ty == "int8" {
                n.as_i64().map(|i| i.to_string()).unwrap_or_else(|| n.to_string())
            } else if ty == "integer" || ty == "int4" || ty == "int" || ty == "smallint" || ty == "int2" {
                n.as_i64().map(|i| i.to_string()).unwrap_or_else(|| n.to_string())
...
mod tests {
    ...
    async fn test_sqlite_to_pg_roundtrip() {
        ...
        let _ = sqlx::query("DROP TABLE IF EXISTS poc_test").execute(&pg_pool).await;
        sqlx::query(
        ...
        ).execute(&pg_pool).await.expect("Failed to create PG table");
        ...
        let row0 = &rows[0];
        let id = row0.iter().find(|(n, _)| n == "id").map(|(_, v)| v.as_str().unwrap_or("")).unwrap();
        assert_eq!(id, "test-1");

        let name = row0.iter().find(|(n, _)| n == "name").map(|(_, v)| v.as_str().unwrap_or("")).unwrap();
        assert_eq!(name, "test-item");

        // metadata: SQLite BLOB → Vec<u8> → parse JSON → Value ✅
        let meta = row0.iter().find(|(n, _)| n == "metadata").map(|(_, v)| v.clone()).unwrap();
        assert!(meta.is_object(), "metadata should be JSON object, got: {:?}", meta);
        assert_eq!(meta["key"], "value");
        assert_eq!(meta["models"][0], "gpt-4");

        // spend: SQLite REAL → f64 ✅
        let spend = row0.iter().find(|(n, _)| n == "spend").map(|(_, v)| v.as_f64()).unwrap();
        assert!((spend.unwrap() - 99.5).abs() < 0.01, "spend should be 99.5");

        // blocked: SQLite INTEGER → i64 → !=0 ✅
        let blocked = row0.iter().find(|(n, _)| n == "blocked").map(|(_, v)| v.as_i64()).unwrap();
        assert_eq!(blocked, Some(0), "blocked should be 0 (false)");

        // blocked=true for test-2: SQLite INTEGER 1 → i64 1 ✅
        let row1 = &rows[1];
        let blocked1 = row1.iter().find(|(n, _)| n == "blocked").map(|(_, v)| v.as_i64()).unwrap();
        ...
        assert_eq!(pg_blocked_0, Some(false), "PG blocked should be false for test-1");

        let pg_blocked_1 = pg_rows[1]
            .iter()
            .find(|(n, _)| n == "blocked")
            .and_then(|(_, v)| v.as_bool());
        assert_eq!(pg_blocked_1, Some(true), "PG blocked should be true for test-2");
    ...
    async fn test_pg_to_sqlite_roundtrip() {
        ...
        // Setup PG source
        let _ = sqlx::query("DROP TABLE IF EXISTS poc_src") .execute(&pg_pool).await;
        sqlx::query(
        ...
        sqlx::query(
            "INSERT INTO poc_src (id, config, active, score) VALUES ($1, $2, $3, $4)"
        )
        .bind("pg-item")
        .bind(&config)
        .bind(true)
        ...
        // PG JSONB → Value ✅
        let pg_config = rows[0].iter().find(|(n, _)| n == "config").map(|(_, v)| v.clone()).unwrap();
        assert!(pg_config.is_object());
        assert_eq!(pg_config["mode"], "production");
        assert_eq!(pg_config["retries"], 3);

        // PG BOOLEAN → bool ✅
        let active = rows[0].iter().find(|(n, _)| n == "active").and_then(|(_, v)| v.as_bool());
        assert_eq!(active, Some(true));

        // PG DOUBLE PRECISION → f64 ✅
        let score = rows[0].iter().find(|(n, _)| n == "score").and_then(|(_, v)| v.as_f64());
        ...
        for row in &rows {
            ...
            let active: i64 = val_map["active"].as_bool().map(|b| if b { 1 } else { 0 }).unwrap_or(0);
        ...
        let sqlite_config = sqlite_rows[0].iter().find(|(n, _)| n == "config").map(|(_, v)| v.clone()).unwrap();
    ...
    async fn test_sqlite_native_type_decode() {
        ...
        sqlx::query(
            "CREATE TABLE t (id TEXT, meta BLOB, count INTEGER, score REAL, ts DATETIME)"
        ...
        // TEXT → String ✅
        let id = row.iter().find(|(n, _)| n == "id").and_then(|(_, v)| v.as_str());
        assert_eq!(id, Some("a"));

        // BLOB → Vec<u8> → parse JSON → Value ✅
        let meta = row.iter().find(|(n, _)| n == "meta").map(|(_, v)| v.clone()).unwrap();
        assert!(meta.is_object());
        assert_eq!(meta["key"], "val");

        // INTEGER → i64 ✅
        let count = row.iter().find(|(n, _)| n == "count").and_then(|(_, v)| v.as_i64());
        assert_eq!(count, Some(42));

        // REAL → f64 ✅
        let score = row.iter().find(|(n, _)| n == "score").and_then(|(_, v)| v.as_f64());
        assert!((score.unwrap() - 3.14).abs() < 0.01);

        // DATETIME → String ✅
        let ts = row.iter().find(|(n, _)| n == "ts").and_then(|(_, v)| v.as_str());
    ...
    async fn test_virtual_keys_sqlite_to_pg_roundtrip() {
        ...
        let _ = sqlx::query("DROP TABLE IF EXISTS virtual_keys").execute(&pg_pool).await;
        sqlx::query(
        ...
        let metadata_json = serde_json::json!({"tags": ["prod"], "user": "admin", "model_group": "openai"});
        ...
        let r0 = &rows[0];
        assert_eq!(r0.iter().find(|(n,_)| n=="token").and_then(|(_,v)| v.as_str()), Some("sk-test-key-001"));
        assert_eq!(r0.iter().find(|(n,_)| n=="blocked").and_then(|(_,v)| v.as_i64()), Some(0));
        assert!((r0.iter().find(|(n,_)| n=="spend").and_then(|(_,v)| v.as_f64()).unwrap() - 99.5).abs() < 0.01);
        assert!(r0.iter().find(|(n,_)| n=="metadata").map(|(_,v)| v.is_object()).unwrap());
        let model_spend = r0.iter().find(|(n,_)| n=="model_spend").map(|(_,v)| v.clone()).unwrap();
        assert_eq!(model_spend["gpt-4"], 15.5);

        // Verify row 1
        let r1 = &rows[1];
        assert_eq!(r1.iter().find(|(n,_)| n=="blocked").and_then(|(_,v)| v.as_i64()), Some(1));
        // SQLite NULL stored as DATETIME — read back as String (empty) or Null depending on driver
        let expires_val = r1.iter().find(|(n,_)| n=="expires").map(|(_,v)| v.clone()).unwrap();
        eprintln!("  DEBUG expires_val={:?}", expires_val);
        assert!(expires_val.is_null() || (expires_val.is_string() && expires_val.as_str().unwrap().is_empty()),
            "expires should be null or empty, got {:?}", expires_val);
        ...
        let pg_meta = pg_rows[0].iter().find(|(n,_)| n=="metadata").map(|(_,v)| v.clone()).unwrap();
        assert!(pg_meta.is_object());
        assert_eq!(pg_meta["user"], "admin");

        // PG BOOLEAN → bool ✅
        assert_eq!(pg_rows[0].iter().find(|(n,_)| n=="blocked").and_then(|(_,v)| v.as_bool()), Some(false));
        assert_eq!(pg_rows[1].iter().find(|(n,_)| n=="blocked").and_then(|(_,v)| v.as_bool()), Some(true));

        // PG DOUBLE PRECISION → f64 ✅
        let spend = pg_rows[0].iter().find(|(n,_)| n=="spend").and_then(|(_,v)| v.as_f64());
        assert!((spend.unwrap() - 99.5).abs() < 0.01);

        // TIMESTAMPTZ: verify row 0 has a value, row 1 NULL is fine
        eprintln!("  DEBUG pg_expires[0]={:?}", pg_rows[0].iter().find(|(n,_)| n=="expires").map(|(_,v)| v.clone()));
        eprintln!("  DEBUG pg_expires[1]={:?}", pg_rows[1].iter().find(|(n,_)| n=="expires").map(|(_,v)| v.clone()));
