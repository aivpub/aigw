...
fn build_router(state: aigw_server::routes::keys::SharedState) -> axum::Router {
    axum::Router::new()
        .route(
            "/global/spend/logs",
...
async fn given_spend_log_with_end_user(world: &mut TestWorld, step: &Step) {
    ...
    let log = SpendLog {
        ...
        response: Some(
            serde_json::json!({"type":"message","content":[{"type":"text","text":"hello"}]}),
        ),
        session_id,
...
async fn then_first_record_has_end_user(world: &mut TestWorld) {
    ...
    let end_user_val = data[0]
        .get("end_user")
        .expect("no end_user field in response");
...
async fn then_first_record_end_user_empty(world: &mut TestWorld) {
    ...
    match raw {
        ...
        Some(v) => {
            ...
            assert!(
                s.is_empty(),
                "Expected end_user to be null or empty, got '{}'",
                s
            );
        }
    }
