...
async fn when_seed_multi_model_and_query(world: &mut TestWorld) {
    ...
    real_db_seed::ensure_virtual_key(&db_url, token, "models-test", None)
        .await
        .ok();
    let hash = aigw_core::crypto::hash_token(token);

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-mdl-")
        .await
        .ok();
    ...
    eprintln!(
        "[DEBUG] /global/spend/models: status={} body={}",
        status,
...
async fn when_seed_date_filter_models(world: &mut TestWorld) {
    ...
    real_db_seed::ensure_virtual_key(&db_url, token, "models-date-test", None)
        .await
        .ok();
    let hash = aigw_core::crypto::hash_token(token);

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-mdt-")
        .await
        .ok();
    ...
    let url = format!(
        "{}/global/spend/models?start_date=2026-07-20&end_date=2026-07-23",
        base
    );
    let client = real_api_steps::client();
    let resp = client
    ...
    let text = resp.text().await.unwrap_or_default();
    eprintln!(
        "[DEBUG] /global/spend/models date-filter: status={} body={}",
        status,
        &text[..text.len().min(500)]
    );
    world.last_status = Some(status);
...
async fn when_seed_multi_provider_and_query(world: &mut TestWorld) {
    ...
    real_db_seed::ensure_virtual_key(&db_url, token, "providers-test", None)
        .await
        .ok();
    let hash = aigw_core::crypto::hash_token(token);

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-prv-")
        .await
        .ok();
    ...
    let mut r2 =
        real_db_seed::SeedRow::new("bdd-prv-b", &hash, 5.0, 100, "gpt-4", "2026-07-20T11:00:00");
    ...
    let text = resp.text().await.unwrap_or_default();
    eprintln!(
        "[DEBUG] /global/spend/providers: status={} body={}",
        status,
        &text[..text.len().min(500)]
    );
    world.last_status = Some(status);
...
async fn then_models_grouped_correct(world: &mut TestWorld) {
    ...
    let gpt4 = arr
        .iter()
        .find(|e| e.get("model").and_then(|v| v.as_str()) == Some("gpt-4"))
        .expect("gpt-4 entry not found");
    let gpt35 = arr
        .iter()
        .find(|e| e.get("model").and_then(|v| v.as_str()) == Some("gpt-3.5"))
    ...
    assert!(
        gpt4_spend >= 10.0,
        "Expected gpt-4 spend>=10.0, got {} (body: {})",
        gpt4_spend,
        body
    );
    assert!(
        gpt4_requests >= 2,
        "Expected gpt-4 requests>=2, got {}",
        gpt4_requests
    );

    // gpt-3.5 should have spend >= 5.0
    let gpt35_spend = gpt35
        .get("total_spend")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    ...
    assert!(
        gpt35_spend >= 5.0,
        "Expected gpt-3.5 spend>=5.0, got {} (body: {})",
        gpt35_spend,
        body
    );
    assert!(
        gpt35_requests >= 1,
        "Expected gpt-3.5 requests>=1, got {}",
        gpt35_requests
    );
}
...
async fn then_models_date_filtered(world: &mut TestWorld) {
    ...
    assert!(
        gpt4_spend >= 6.0,
        "Expected date-filtered gpt-4 spend>=6.0, got {} (body: {})",
        gpt4_spend,
        body
    );
    assert!(
        gpt4.get("requests").and_then(|v| v.as_i64()).unwrap_or(0) >= 1,
