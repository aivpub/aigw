...
async fn when_seed_and_query_rankings(world: &mut TestWorld) {
    ...
    real_db_seed::ensure_virtual_key(&db_url, token_a, "ranking-a", None)
        .await
        .expect("ensure virtual key a");
    real_db_seed::ensure_virtual_key(&db_url, token_b, "ranking-b", None)
        .await
    ...
    real_db_seed::cleanup_by_prefix(&db_url, "bdd-rank-")
        .await
...
async fn then_rankings_first_max_with_alias(world: &mut TestWorld) {
    ...
    let spend_a = entry_a
        .and_then(|e| e.get("total_spend").and_then(|v| v.as_f64()))
        .unwrap_or(0.0);
    let spend_b = entry_b
        .and_then(|e| e.get("total_spend").and_then(|v| v.as_f64()))
        .unwrap_or(0.0);
    ...
    assert!(
        (spend_a - 13.0).abs() < 0.01,
        "Expected ranking-a spend=13.0, got {} (body: {})",
        spend_a,
        body
    );
    assert!(
        (spend_b - 5.0).abs() < 0.01,
        "Expected ranking-b spend=5.0, got {} (body: {})",
        spend_b,
        body
    );
    ...
    let ka = entry_a
        .and_then(|e| e.get("key_alias").and_then(|v| v.as_str()))
        .unwrap_or("");
    assert!(
        !ka.is_empty(),
        "key_alias for ranking-a should not be empty"
    );
    let kb = entry_b
        .and_then(|e| e.get("key_alias").and_then(|v| v.as_str()))
        .unwrap_or("");
    assert!(
        !kb.is_empty(),
        "key_alias for ranking-b should not be empty"
    );
}
