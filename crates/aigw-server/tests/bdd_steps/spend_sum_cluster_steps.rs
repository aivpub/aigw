...
async fn when_seed_and_query_global_spend(world: &mut TestWorld) {
    ...
    real_db_seed::ensure_virtual_key(&db_url, token, "sum-global-test", None).await.ok();
    let hash = aigw_core::crypto::hash_token(token);

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-sgl-").await.ok();

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-").await.ok();
    real_db_seed::cleanup_keys_by_alias(&db_url, "sum-%").await.ok();

    let rows = vec![
        real_db_seed::SeedRow::new("bdd-sgl-a", &hash, 5.0, 100, "gpt-4", "2026-07-20T10:00:00"),
        real_db_seed::SeedRow::new("bdd-sgl-b", &hash, 7.0, 200, "gpt-3.5", "2026-07-20T11:00:00"),
    ];
    ...
    let resp = client.get(&url)
        ...
        .send().await.expect("global/spend request failed");
...
async fn when_seed_user_and_query_spend_users(world: &mut TestWorld) {
    ...
    real_db_seed::cleanup_by_prefix(&db_url, "bdd-usr-").await.ok();

    let mut r1 = real_db_seed::SeedRow::new("bdd-usr-a", &hash, 7.0, 100, "gpt-4", "2026-07-20T10:00:00");
    r1.user = Some("test-user-76".to_string());
    let mut r2 = real_db_seed::SeedRow::new("bdd-usr-b", &hash, 8.0, 100, "gpt-4", "2026-07-20T11:00:00");
    ...
    let client = real_api_steps::client();
    let resp = client.get(&spend_url)
        .header("Authorization", format!("Bearer {}", raw_key))
        .send().await.expect("spend/users request failed");
...
async fn when_seed_tags_and_query(world: &mut TestWorld) {
    ...
    real_db_seed::ensure_virtual_key(&db_url, token, "sum-tags-test", None).await.ok();
    let hash = aigw_core::crypto::hash_token(token);

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-tag-").await.ok();
    ...
    let mut r1 = real_db_seed::SeedRow::new("bdd-tag-a", &hash, 10.0, 100, "gpt-4", "2026-07-20T10:00:00");
    r1.request_tags = Some("important".to_string());
    let mut r2 = real_db_seed::SeedRow::new("bdd-tag-b", &hash, 10.0, 100, "gpt-4", "2026-07-20T11:00:00");
    r2.request_tags = Some("important".to_string());
    let r3 = real_db_seed::SeedRow::new("bdd-tag-c", &hash, 50.0, 200, "gpt-4", "2026-07-20T12:00:00");
    ...
    let resp = client.get(&url)
        .header("Authorization", format!("Bearer {}", mk))
        .send().await.expect("spend/tags request failed");
...
async fn when_seed_multi_key_and_query_global_keys(world: &mut TestWorld) {
    if !seed_enabled() {
        real_api_steps::set_skip_pass(world, 200, serde_json::json!({"data": [
    ...
    real_db_seed::ensure_virtual_key(&db_url, token_a, "sum-keys-a", None).await.ok();
    real_db_seed::ensure_virtual_key(&db_url, token_b, "sum-keys-b", None).await.ok();
    let hash_a = aigw_core::crypto::hash_token(token_a);
    let hash_b = aigw_core::crypto::hash_token(token_b);

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-gk-").await.ok();

    let rows = vec![
        real_db_seed::SeedRow::new("bdd-gk-a1", &hash_a, 3.0, 50, "gpt-4", "2026-07-20T10:00:00"),
        real_db_seed::SeedRow::new("bdd-gk-b1", &hash_b, 3.0, 50, "gpt-3.5", "2026-07-20T10:30:00"),
        real_db_seed::SeedRow::new("bdd-gk-b2", &hash_b, 5.0, 100, "gpt-3.5", "2026-07-20T11:00:00"),
    ];
    real_db_seed::seed_spend_logs(&db_url, &rows).await.expect("seed");
    ...
    let resp = client.get(&url)
        .header("Authorization", format!("Bearer {}", mk))
        .send().await.expect("global/spend/keys request failed");
...
async fn then_global_spend_correct(world: &mut TestWorld) {
    ...
    assert!((spend - 12.0).abs() < 0.01,
        "Expected spend=12.0, got {} (body: {})", spend, body);
}
...
async fn then_spend_users_correct(world: &mut TestWorld) {
    ...
    let spend = body.get("spend").and_then(|v| v.as_f64()).unwrap_or(0.0);
    assert!((spend - 15.0).abs() < 0.01,
        "Expected user spend=15.0, got {} (body: {})", spend, body);
}
...
async fn then_spend_tags_correct(world: &mut TestWorld) {
    ...
    let spend = body.get("spend").and_then(|v| v.as_f64()).unwrap_or(0.0);
    assert!((spend - 20.0).abs() < 0.01,
        "Expected tag 'important' spend=20.0, got {} (body: {})", spend, body);
}
...
async fn then_global_keys_correct(world: &mut TestWorld) {
    ...
    let key_a_entry = arr.iter().find(|e| {
        e.get("api_key").and_then(|v| v.as_str()) == Some(&hash_a)
    });
    let key_b_entry = arr.iter().find(|e| {
        e.get("api_key").and_then(|v| v.as_str()) == Some(&hash_b)
    });

    let spend_a = key_a_entry.and_then(|e| e.get("spend").and_then(|v| v.as_f64())).unwrap_or(0.0);
    let spend_b = key_b_entry.and_then(|e| e.get("spend").and_then(|v| v.as_f64())).unwrap_or(0.0);

    assert!((spend_a - 3.0).abs() < 0.01,
        "Expected key_a spend=3.0, got {} (body: {})", spend_a, body);
    assert!((spend_b - 8.0).abs() < 0.01,
        "Expected key_b spend=8.0, got {} (body: {})", spend_b, body);
}
...
async fn when_tags_no_param(world: &mut TestWorld, alias: String) {
    if !real_api_steps::real_api_enabled() {
        real_api_steps::set_skip_pass(world, 400, serde_json::json!({"error": {"message": "missing tag parameter"}}));
        return;
    }
    ...
    eprintln!("[DEBUG] /spend/tags no-param status={} body={:?}",
        resp.status().as_u16(),
