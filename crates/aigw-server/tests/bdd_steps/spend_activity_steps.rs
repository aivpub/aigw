...
async fn when_seed_cross_day_and_query_activity(world: &mut TestWorld) {
    ...
    let mut r3 = real_db_seed::SeedRow::new(
        "bdd-act-d2a",
        &hash,
        4.0,
        100,
    ...
    let mut r4 = real_db_seed::SeedRow::new(
        "bdd-act-d2b",
        &hash,
        7.0,
        100,
    ...
    real_db_seed::seed_spend_logs(&db_url, &[r1, r2, r3, r4])
        .await
        .expect("seed");
    ...
    let resp = client
        ...
        .send()
        .await
        .expect("activity request failed");
...
async fn when_seed_user_filter_activity(world: &mut TestWorld) {
    ...
    real_db_seed::ensure_virtual_key(&db_url, token, "activity-user-test", None)
        .await
        .ok();
    let hash = aigw_core::crypto::hash_token(token);

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-ausr-")
        .await
        .ok();
    ...
    let url = format!(
        "{}/global/spend/activity?start_date=2026-07-20&end_date=2026-07-21&user_id=user-alice",
        base
    );
    let client = real_api_steps::client();
    let resp = client
...
async fn when_seed_team_filter_activity(world: &mut TestWorld) {
    ...
    real_db_seed::ensure_virtual_key(&db_url, token, "activity-team-test", None)
        .await
        .ok();
    let hash = aigw_core::crypto::hash_token(token);

    real_db_seed::cleanup_by_prefix(&db_url, "bdd-atm-")
        .await
        .ok();

    let mut r1 =
        real_db_seed::SeedRow::new("bdd-atm-a", &hash, 7.0, 100, "gpt-4", "2026-07-20T10:00:00");
    r1.team_id = Some("team-red".to_string());
    let mut r2 =
        real_db_seed::SeedRow::new("bdd-atm-b", &hash, 8.0, 100, "gpt-4", "2026-07-20T11:00:00");
    ...
    let url = format!(
        "{}/global/spend/activity?start_date=2026-07-20&end_date=2026-07-21&team_id=team-red",
        base
    );
    let client = real_api_steps::client();
    let resp = client
...
async fn then_activity_metadata_correct(world: &mut TestWorld) {
    ...
    let total_spend = meta
        .get("total_spend")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let total_requests = meta
        .get("total_requests")
        .and_then(|v| v.as_i64())
        .unwrap_or(0);
    let successful_requests = meta
        .get("successful_requests")
        .and_then(|v| v.as_i64())
        .unwrap_or(0);
    let failed_requests = meta
        .get("failed_requests")
        .and_then(|v| v.as_i64())
        .unwrap_or(0);
    ...
    assert!(
        (total_spend - 22.0).abs() < 0.01,
        "Expected total_spend=22.0, got {}",
        total_spend
    );
    assert_eq!(
        total_requests, 4,
        "Expected total_requests=4, got {} (body: {:?})",
        total_requests, body
    );
...
async fn then_activity_daily_correct(world: &mut TestWorld) {
    ...
    assert_eq!(
        granularity, "daily",
        "Expected granularity=daily for >3-day range, got {}",
        granularity
    );
    ...
    let d1 = &daily[0];
    assert_eq!(
        d1.get("date").and_then(|v| v.as_str()).unwrap_or(""),
        "2026-07-20"
    );
    let d1_spend = d1
        .get("spend")
        .or(d1.get("total_spend"))
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    assert!(
        (d1_spend - 11.0).abs() < 0.01,
        "Expected day 1 spend=11.0, got {} (daily: {:?})",
        d1_spend,
        d1
    );
    ...
    let d2 = &daily[1];
    assert_eq!(
        d2.get("date").and_then(|v| v.as_str()).unwrap_or(""),
        "2026-07-21"
    );
    let d2_spend = d2
        .get("spend")
        .or(d2.get("total_spend"))
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    assert!(
        (d2_spend - 11.0).abs() < 0.01,
        "Expected day 2 spend=11.0, got {} (daily: {:?})",
        d2_spend,
        d2
    );
}
...
async fn then_activity_user_filter_correct(world: &mut TestWorld) {
    ...
    assert!(
        (total_spend - 10.0).abs() < 0.01,
        "Expected user-alice spend=10.0, got {}",
        total_spend
    );
...
async fn then_activity_team_filter_correct(world: &mut TestWorld) {
    ...
    assert!(
        (total_spend - 15.0).abs() < 0.01,
        "Expected team-red spend=15.0, got {}",
        total_spend
    );
...
async fn when_activity_nonadmin(world: &mut TestWorld, alias: String) {
    if !real_api_steps::real_api_enabled() {
        real_api_steps::set_skip_pass(
            world,
            403,
            serde_json::json!({"error": {"message": "admin required"}}),
        );
        return;
    }
    let token = world
